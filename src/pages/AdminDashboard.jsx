...
const OrderStatusSelect = ({ orderId, initialStatus, onStatusChange }) => {
  ...
  const statuses = [
    ...
    { value: 'processing', label: 'En préparation', color: 'bg-purple-50 text-purple-700 border-purple-200' },
    ...
    { value: 'cancelled', label: 'Annulée', color: 'bg-red-50 text-red-700 border-red-200' }
  ];

  // Map old string statuses if needed
  const normalizedStatus = status === 'En_attente' ? 'pending' : status === 'En_preparation' ? 'processing' : status === 'Expédiée' ? 'shipped' : status === 'Livrée' ? 'delivered' : status === 'Annulée' ? 'cancelled' : status;
  
  const currentCfg = statuses.find(s => s.value === normalizedStatus) || statuses[0];
  ...
  return (
    <div className="relative inline-flex items-center">
      <select
        ...
        disabled={isLoading}
        className={`appearance-none pl-3 pr-8 py-1 rounded-full text-xs font-semibold border focus:outline-none focus:ring-2 focus:ring-primary/20 transition-colors cursor-pointer ${currentCfg.color} ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
      >
        {statuses.map(s => (
                ...
                <tbody className="divide-y divide-gray-50">
                  {sortedOrders.map((order, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <OrderStatusSelect orderId={order.id} initialStatus={order.status} onStatusChange={handleStatusChange} />
                      </td>
                      <td className="px-6 py-4">
                        <button
